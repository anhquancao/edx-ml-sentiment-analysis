...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    # Your code here
    loss = 0
    for r in range(feature_matrix.shape[0]):
        row = feature_matrix[r, :]
        label = labels[r]
        loss += hinge_loss_single(row, label, theta, theta_0)
...
def pegasos_single_step_update(
    ...
    """
    # Your code here
    y = np.dot(current_theta, feature_vector) + current_theta_0
    if y * label <= 1:
